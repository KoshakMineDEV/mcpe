...
var ASM_CONSTS = {
 ...
 247216: () => window.innerWidth,
 247246: () => window.innerHeight,
 247277: ($0, $1) => {
...
function _alGetString(param) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(28, 1, param);
...
function _alIsBuffer(bufferId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 1, bufferId);
 if (!AL.currentCtx) {
  return false;
 }
...
function _alIsSource(sourceId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 1, sourceId);
 if (!AL.currentCtx) {
  return false;
 }
...
function _alListener3f(param, value0, value1, value2) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 1, param, value0, value1, value2);
 switch (param) {
 case 4100:
 case 4102:
...
function _alListenerf(param, value) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(32, 1, param, value);
 switch (param) {
 case 4106:
...
function _alListenerfv(param, pValues) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(33, 1, param, pValues);
 if (!AL.currentCtx) {
  return;
 }
 if (!pValues) {
...
function _alSource3f(sourceId, param, value0, value1, value2) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 1, sourceId, param, value0, value1, value2);
 switch (param) {
 case 4100:
 case 4101:
 case 4102:
...
function _alSourcePlay(sourceId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 1, sourceId);
 if (!AL.currentCtx) {
  return;
 }
 var src = AL.currentCtx.sources[sourceId];
...
function _alSourcef(sourceId, param, value) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(36, 1, sourceId, param, value);
 switch (param) {
 case 4097:
 /* AL_CONE_INNER_ANGLE */ case 4098:
...
function _alcCloseDevice(deviceId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(37, 1, deviceId);
...
function _alcCreateContext(deviceId, pAttrList) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(38, 1, deviceId, pAttrList);
...
function _alcDestroyContext(contextId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(39, 1, contextId);
...
function _alcGetError(deviceId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(40, 1, deviceId);
 var err = AL.alcErr;
 AL.alcErr = 0;
 return err;
}
...
function _alcGetString(deviceId, param) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(41, 1, deviceId, param);
...
function _alcMakeContextCurrent(contextId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(42, 1, contextId);
 if (contextId === 0) {
  AL.currentCtx = null;
 } else {
...
function _alcOpenDevice(pDeviceName) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(43, 1, pDeviceName);
 if (pDeviceName) {
...
function _eglBindAPI(api) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(44, 1, api);
...
function _eglChooseConfig(display, attrib_list, configs, config_size, numConfigs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(45, 1, display, attrib_list, configs, config_size, numConfigs);
...
function _eglCreateContext(display, config, hmm, contextAttribs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(46, 1, display, config, hmm, contextAttribs);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglCreateWindowSurface(display, config, win, attrib_list) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(47, 1, display, config, win, attrib_list);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglDestroyContext(display, context) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(48, 1, display, context);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglDestroySurface(display, surface) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(49, 1, display, surface);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglGetConfigAttrib(display, config, attribute, value) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(50, 1, display, config, attribute, value);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglGetDisplay(nativeDisplayType) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(51, 1, nativeDisplayType);
 EGL.setErrorCode(12288);
...
function _eglInitialize(display, majorVersion, minorVersion) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(53, 1, display, majorVersion, minorVersion);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglMakeCurrent(display, draw, read, context) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(54, 1, display, draw, read, context);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglQueryString(display, name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(55, 1, display, name);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglSwapBuffers(dpy, surface) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(56, 1, dpy, surface);
...
function _eglSwapInterval(display, interval) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(57, 1, display, interval);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglTerminate(display) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(58, 1, display);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglWaitNative(nativeEngineId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(60, 1, nativeEngineId);
 EGL.setErrorCode(12288);
 /* EGL_SUCCESS */ return 1;
}
...
function _emscripten_get_element_css_size(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(66, 1, target, width, height);
...
function _emscripten_get_gamepad_status(index, gamepadState) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(67, 1, index, gamepadState);
...
function _emscripten_get_preloaded_image_data(path, w, h) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(69, 1, path, w, h);
...
function _emscripten_request_fullscreen_strategy(target, deferUntilInEventHandler, fullscreenStrategy) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(71, 1, target, deferUntilInEventHandler, fullscreenStrategy);
 var strategy = {
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(72, 1, target, deferUntilInEventHandler);
 target = findEventTarget(target);
 if (!target) return -4;
 if (!target.requestPointerLock) {
  return -1;
...
function _emscripten_set_beforeunload_callback_on_thread(userData, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(74, 1, userData, callbackfunc, targetThread);
...
function _emscripten_set_element_css_size(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(76, 1, target, width, height);
 target = findEventTarget(target);
 if (!target) return -4;
...
function _emscripten_set_gamepadconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(79, 1, userData, useCapture, callbackfunc, targetThread);
 if (!navigator.getGamepads || disableGamepadApiIfItThrows()) return -1;
 return registerGamepadEventCallback(2, userData, useCapture, callbackfunc, 26, "gamepadconnected", targetThread);
}

function _emscripten_set_gamepaddisconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(80, 1, userData, useCapture, callbackfunc, targetThread);
 if (!navigator.getGamepads || disableGamepadApiIfItThrows()) return -1;
...
function _emscripten_set_visibilitychange_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(95, 1, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_window_title(title) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(97, 1, title);
...
var _environ_get = function(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(98, 1, __environ, environ_buf);
 var bufSize = 0;
...
var _environ_sizes_get = function(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(99, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(100, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(102, 1, fd, offset_low, offset_high, whence, newOffset);
...
function _gethostbyname(name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(104, 1, name);
